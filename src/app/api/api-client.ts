...
export class ApiClient implements IApiClient {
    ...
    cartItem(body: CartItem | undefined): Observable<CartItem> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processCartItem(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processCartItem(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<CartItem>;
                }
            } else
    ...
    protected processCartItem(response: HttpResponseBase): Observable<CartItem> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
